...
Define the existing Alarisa route as a current-work direction and continuity candidate.
...
The current page calls Alarisa a major ongoing multi-component system, names TeqFW and ADSM as foundation, states that its exploration is experimental and incomplete, and links PDE as a related current system. It provides one safe external repository link as inspectable system evidence, not seven equal public cards or a product-readiness claim. It has no false purchase CTA. Any commercial link leads only to an independently valid offer or direct human handoff with status intact. There are no fabricated Alarisa Event relationships when the authored Journal does not support them.
