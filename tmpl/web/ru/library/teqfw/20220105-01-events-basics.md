...
обработчиков событий:
    ...
    unsubscribe(subscription) {} Подписчик сообщает генератору, на какое его событие он хочет получать сообщения, и предоставляет генератору обработчик для реакции на событие. Когда генератор фиксирует наступление нужного события, он отправляет сообщение с деталями события обработчику:
