...
test('adds localized canonical metadata and avoids enriching unrelated pages', async () => {
    const adapter = new Adapter({path: {extname: (value) => value.endsWith('.html') ? '.html' : '', posix: {}}, cmsAdapter: {getRenderData: async () => ({target: {}, data: {}})}, helpWeb: {extractRoutingInfo: () => ({locale: 'en', cleanPath: '/'})}, metadata: new Metadata({config: {getBaseUrl: () => 'https://wiredgeese.com'}, tmplConfig: {getAvailableLocales: () => ['en', 'ru']}}), tmplConfig: {getAvailableLocales: () => ['en', 'ru'], getDefaultLocale: () => 'en'}, logger: {forSource: () => ({warn() {}, error() {}})}, blogHandler: {collectRecentBlogEntries: async () => []}});
