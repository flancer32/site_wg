...
- Localized public pages must preserve the canonical English document structure unless a locale-specific structural difference is explicitly required and documented. This includes section boundaries and order, wrappers, CSS classes, IDs, component structure, link intent, and template logic.
...
- Give translation subagents only the context required for accurate localization:
  - the complete final canonical English template being translated, rather than isolated text fragments;
  - the target locale;
...
- Translation subagents must preserve accepted product semantics, claims, CTA intent, status, and boundaries, as well as section boundaries and order, wrappers, CSS classes, IDs, component structure, link intent, and template logic. They may adapt locale-specific wording idiomatically, but must not introduce new positioning, offers, promises, capabilities, interpretations, or independent page restructuring.
...
- For substantial multilingual changes, use a separate parity-review pass after translation. The reviewer should compare canonical English with each translated locale for both semantic parity (semantic drift, missing claims, altered CTA meaning, product-status differences, and accidental additions) and structural parity (section order, wrappers, classes, IDs, components, links, and template logic).
